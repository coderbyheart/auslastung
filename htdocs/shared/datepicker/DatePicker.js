 ...
 * @modified by Markus Tacker <m@tacker.org> to work with Mootools 1.2.1
 * @version $Id: DatePicker.js 9 2009-02-24 12:10:25Z m $
...
var DatePicker = new Class({
	...
	initialize: function(dp, params){
		...
		if ( typeof params !== 'undefined' ) for( var i in params ) this[ i ] = params[ i ];
		...
		if(dp.value != '') {
			dp.today = new Date();
			dp.then = new Date(
				parseInt( dp.value.substr( this.format.indexOf( 'yyyy' ), 4 ), 10 ),
				parseInt( dp.value.substr( this.format.indexOf( 'mm' ), 2 ), 10 ) - 1,
				parseInt( dp.value.substr( this.format.indexOf( 'dd' ), 2 ), 10 )
			);
		} else {
